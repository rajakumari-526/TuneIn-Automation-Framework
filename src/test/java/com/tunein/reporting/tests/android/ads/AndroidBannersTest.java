...
public class AndroidBannersTest extends BannersTest {
    ...
    public void testBannerDeviceInfo() {
        ...
        reportingAsserts.validateThatAdsParamIsNotPresent(requiredRequest, MAX_BANNER_REQUEST_JSON_IDFA);
    }
    ...
    public void testPrerollDisabledStationBannerFlow() {
        ...
        String verifyURL = getUrlFromRequest(IMA_REQUEST);
        reportingAsserts.validateThatUrlIsAbsent(verifyURL);
        verifyURL = getUrlFromRequest(ADS_WIZZ_PREROLL_REQUEST);
        reportingAsserts.validateThatUrlIsAbsent(verifyURL);
        RequestType[] events = {REPORTS_AD_REQUEST, REPORTS_AD_IMPRESSION};

        for (RequestType event : events) {
